...
/**
 * Comprehensive live test of OpenCode Java SDK Tests all endpoints against a real server
 */
public class ComprehensiveLiveTest {
	...
	public static void main(String[] args) {
		...
		client = OpenCodeClient.builder().baseUrl(BASE_URL).timeout(10000).build();
	...
	private static void testDocumentationService() {
		...
		testEndpoint("GET /doc", "Get OpenAPI documentation", () -> {
			...
			return doc != null && doc.contains("openapi") ? "Success - Doc length: " + doc.length()
					: "Failed - Invalid doc";
		});
	...
	private static void testSessionService() {
		...
		try {
			Session session = client.getSessions()
				.createSession(SessionCreateRequest.builder().title("Comprehensive Test Session").build());
		...
		if (sessionId != null) {
			...
			testEndpoint("GET /session", "List sessions", () -> {
				...
				return sessions != null && !sessions.isEmpty() ? "Success - Sessions: " + sessions.size() : "Failed";
			});
			...
			testEndpoint("PATCH /session/{id}", "Update session", () -> {
				...
				return s != null && "Updated Test Session".equals(s.getTitle()) ? "Success - Updated" : "Failed";
			});
			...
			testEndpoint("POST /session/{id}/init", "Initialize session", () -> {
				client.getSessions().initializeSession(sid, "opencode", "grok-code");
			...
			testEndpoint("POST /session/{id}/shell", "Execute shell", () -> {
				Message result = client.getSessions()
					.executeShellCommand(sid, ShellRequest.builder().command("echo 'test'").agent("shell").build());
	...
	private static void testCommandService() {
		...
		testEndpoint("GET /agent", "List agents", () -> {
			...
			return agents != null && !agents.isEmpty() ? "Success - Agents: " + agents.size() : "Failed - No agents";
		});
	...
	private static void testToolService() {
		...
		testEndpoint("GET /experimental/tool/ids", "List tool IDs", () -> {
			...
			return ids != null && !ids.isEmpty() ? "Success - Tool IDs: " + ids.size() : "Failed - No tools";
		});

		testEndpoint("GET /experimental/tool", "List tools", () -> {
			List<Tool> tools = client.getTools().listTools("opencode", "grok-code");
			return tools != null && !tools.isEmpty() ? "Success - Tools: " + tools.size() : "Failed - No tools";
		});
	...
	private static void testTuiService() {
		...
		String[] tuiEndpoints = { "POST /tui/append-prompt", "POST /tui/submit-prompt", "POST /tui/clear-prompt",
				"POST /tui/open-help", "POST /tui/open-sessions", "POST /tui/open-themes", "POST /tui/open-models",
				"POST /tui/execute-command", "POST /tui/show-toast" };
	...
	private static void saveReport() {
		try {
			Files.write(Paths.get("COMPREHENSIVE_TEST_REPORT.md"), report.toString().getBytes());
